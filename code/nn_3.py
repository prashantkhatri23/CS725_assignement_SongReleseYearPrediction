...
def eig_vectors(x,x_scaler):
  """
  returns eigenvectors of the input feature matrix
  """
  x_scaled = transform_standard_scaler_fun(x,x_scaler)
  features = x_scaled.T
...
class Net(object):
  ...
  def __init__(self, num_layers, num_units,NUM_FEATS_pc):
    '''
    ...
    '''
    self.NUM_FEATS_pc = NUM_FEATS_pc
    ...
    for i in range(self.num_layers):
      if i==0:
        # Input layer
        self.weights.append(np.random.uniform(-1, 1, size=(self.NUM_FEATS_pc, self.num_units)))
  ...
  def __call__(self, X):
    ...
    X = np.dot(X.reshape((-1, NUM_FEATS_orig)), self.p_eigen_vec[:,:self.NUM_FEATS_pc].reshape((NUM_FEATS_orig, -1)))
    # print(X.shape)
    Lrelu_slope = 0
    a = X
...
def train(net, optimizer, lamda, batch_size, max_epochs,
  ...
  net.p_eigen_vec = eig_vectors(train_input,net.scaler_X)
...
def main():
  ...
  Up_c = 4
  per_x_epoch = 2
  NUM_FEATS_pc = 80 # number of pca components chosen

  train_input, train_target, dev_input, dev_target, test_input = read_data()
  net = Net(num_layers, num_units,NUM_FEATS_pc)
